...
if ~isempty(data)
    ...
    ax.Children(7).set('Ydata',[ax.Children(7).YData(size(data,1)+1:end) 1 + [diff(data(:,2)') 0]]); % frames
    ax.Children(6).set('Ydata',[ax.Children(6).YData(size(data,1)+1:end) 3 + data(:,9)']); % reward valve
